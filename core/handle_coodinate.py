...
def transform_coordinate(basis_vector_list, original_basis_vector_list=None):
    ...
    Returns
    -------
    transformer: Callable[[List[Number]], List[Number]]
    ...
    new_basis = [[1,1],[0,1]]
    transformer = transform_coordinate(new_basis, original_basis)

    original_coordinate = [1,1]
    new_coordinate = transformer(original_coordinate)
    ...
    original = np.eye(
        *new_basis.shape) if original_basis_vector_list is None else basis_vector_matrix(*original_basis_vector_list)
...
def basis_by_North(clockwise_degree_from_N1, clockwise_degree_from_N2):
    ...
    return [
        [np.cos(degree_to_radiun(clockwise_degree_from_N1)),
         np.sin(degree_to_radiun(clockwise_degree_from_N1))],
        [np.cos(degree_to_radiun(clockwise_degree_from_N2)),
         np.sin(degree_to_radiun(clockwise_degree_from_N2))]
    ]
